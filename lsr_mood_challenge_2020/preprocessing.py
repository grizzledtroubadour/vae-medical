...
class img_dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.sample: # 采用取样方式读取数据
            sampled_img_batch = []
            for img_ext in img_batch:

                if self.sample_anomaly == 'normal': # 在正常样本中采样
                ...
                elif self.sample_anomaly == 'abnormal': # 在存在异常样本中采样
                ...
                elif self.sample_anomaly is None: # 在所有非空图像中采样
                    msk_empty = ~np.all(img_ext.img == 0,axis=(1,2)) # Remove empty planes
                    msk_empty[:self.slice_offset] = False
                    msk_empty[-self.slice_offset:] = False
                    choices = np.arange(len(msk_empty))[msk_empty] # 获取True对应的位置
                else:
                    raise NotImplementedError

                # np.random does not work inside the dataloader
                sample_idx = np.array(random.choices(choices,k = self.sample_number)) # 随机采样目标个数
                img = img_ext.img[sample_idx].astype(np.float32) # 这里又换成了float32   图像应该是0～256取值范围？
                ...
                img[non_cero_mask] = (img[non_cero_mask] - mu)/std # 均值0，方差1

                # Coordinates in range [-.5,.5] for conditionning
                coord = sample_idx[:, np.newaxis] / img_ext.img.shape[0] # 图像深度归一化到[-.5,.5]
                coord = coord - 0.5
