...
class MSISBI2015Train(MSISBI2015Base):
    NAME = "MSISBI2015/train"
    URL = ""
    FILES = [
        "training_final_v4.zip",
    ]
    SIZES = [
        824324656,
    ]
    ...
    def _prepare(self):
        ...
        if not bdu.is_prepared(self.root):
            ...
            # list
            filelist = glob.glob(os.path.join(datadir, "training*", "orig/*_flair.nii.gz"))
            filelist = [os.path.relpath(p, start=datadir) for p in filelist] # 绝对路径转为相对路径
            filelist = sorted(filelist)
            filelist = [p +';'+
                        p.replace('orig', 'preprocessed')[:-3] +';'+
                        p.replace('orig', 'masks')[:-10]+"_mask1.nii" +';'+
                        p.replace('orig', 'masks')[:-10]+"_mask2.nii" for p in filelist] # 未完成！
            filelist = "\n".join(filelist)+"\n"
...
class MSISBI2015Toy(MSISBI2015Base):
    NAME = "MOODBrain_toy"
