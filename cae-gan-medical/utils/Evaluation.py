    ...
    for p, patient in enumerate(patients):
        ...
        for n, nii_filename in enumerate(filtered_files):
            if len(_eval_dict['diffs']) == 0:
                ...
                nii, nii_seg = datasetObj.load_volume_and_groundtruth(nii_filename, patient)
                ...
                for s in range(slice_start, slice_end):
                    ...
                    slice_skullmap = np.ones(slice_data.shape)
                    ...
                    if (datasetObj.options.useCrops and datasetObj.options.cropType == 'center'):
                        x = crop_center(slice_data, datasetObj.options.cropWidth, datasetObj.options.cropHeight)
                        x = np.expand_dims(x, 2)
                        labelmaps = crop_center(slice_seg, datasetObj.options.cropWidth, datasetObj.options.cropHeight)
                        labelmaps = np.expand_dims(labelmaps, 2)
                    ...
                    for i in range(num_samples):
                        ...
                        x_recs += [np.reshape(apply_brainmask(x_rec_tmp, slice_skullmap, erode=should(options, "erodeBrainmask")), [1, *datasetObj.options.sliceResolution, 1])]
                        x_diffs += [np.reshape(apply_brainmask(np.maximum(x - x_rec_tmp, 0), slice_skullmap, erode=should(options, "erodeBrainmask")), [1, *datasetObj.options.sliceResolution, 1])]
                        # x_recs += [np.reshape(x_rec_tmp, [1, *datasetObj.options.sliceResolution, 1])]
                        # x_diffs += [np.reshape(np.maximum(x - x_rec_tmp, 0), [1, *datasetObj.options.sliceResolution, 1])]
                    ...
                    x_diff = np.reshape(apply_brainmask(x_diff, slice_skullmap, erode=should(options, "erodeBrainmask")), [1, *datasetObj.options.sliceResolution, 1])
                    x_diff = np.reshape(x_diff, [1, *datasetObj.options.sliceResolution, 1])
                    ...
                    imwrite(os.path.join(sampleDir, '{}_{}_rec_variance_combined.png'.format(p, s)), np.squeeze(utils.apply_colormap(x_recs_var, plt.cm.jet)))
                    if x_log_vars.size > 0:
...
def evaluate(datasetPC, gan, options, epoch='last', description=None):
    ...
    eval_pc, patients_pc = _evaluate(datasetPC, gan, sample_dir, options, split="VAL")
    ...
    for p, patient in enumerate(patients_pc):
        ...
        subvolume_groundtruth += 1
        print("subvolume_prediction.shape: ", subvolume_prediction.shape)
        print("subvolume_groundtruth.shape: ", subvolume_groundtruth.shape)
