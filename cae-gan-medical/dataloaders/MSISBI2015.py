...
class MSISBI2015(object):
    ...
    def __init__(self, options=Options()):
        ...
        if options.cache and os.path.isfile(self.pckl_name()):
            ...
            if not os.path.isfile(self.split_name()):
                ...
                print("_numPatients: ", _numPatients)
    ...
    def _create_numpy_arrays(self):
        ...
        for p, patient in enumerate(self.patients):
            print(p, patient)
            print(self.patientsSplit['TRAIN'])
            print(self.patientsSplit['VAL'])
            print(self.patientsSplit['TEST'])
            if (p+1) in self.patientsSplit['TRAIN']:
                _set_of_current_patient = MSISBI2015.SET_TYPES.index('TRAIN')
            elif (p+1) in self.patientsSplit['VAL']:
                _set_of_current_patient = MSISBI2015.SET_TYPES.index('VAL')
            elif (p+1) in self.patientsSplit['TEST']:
        ...
        print("self._images.ndim: ", self._images.ndim)
        if self._images.ndim < 4:
            self._images = numpy.expand_dims(self._images, 3)
        self._sets = numpy.array(_sets).astype(numpy.int32)
        print("self._images 1: ", self._images.shape)
        print("self._labels 1: ", self._labels.shape)
        print("self._sets 1: ", self._sets.shape)
        print(self._sets)
    ...
    def gather_data(self, patient, nii_filename):
        ...
        nii, nii_seg = self.load_volume_and_groundtruth(nii_filename, patient)
        ...
        for s in xrange(self.options.sliceStart, min(self.options.sliceEnd, nii.num_slices_along_axis(self.options.axis))):
            ...
            if self.options.sliceResolution is not None: # 这里面有问题
            ...
            if self.options.useCrops:
                if self.options.cropType == 'random':
                    rx = numpy.random.randint(0, high=(slice_data.shape[1] - self.options.cropWidth), size=self.options.numRandomCropsPerSlice)
                    ry = numpy.random.randint(0, high=(slice_data.shape[0] - self.options.cropHeight), size=self.options.numRandomCropsPerSlice)
                ...
                for protocol, protocol_array in MSISBI2015.PROTOCOL_MAPPINGS.items():
                    if len(options.filterProtocols) > 0 and protocol not in options.filterProtocols:
                        ...
                        if options.format == "raw":
                            ...
                            patient[protocol] = os.path.join(options.dir, folder, "preprocessed", patient['name'] + '_' + protocol_array[0] + '.aligned.nii.gz')
    ...
    def next_batch(self, batch_size, shuffle=True, set='TRAIN', return_brainmask=True):
        ...
        _setIdx = MSISBI2015.SET_TYPES.index(set)
        print("self._sets 2: ", self._sets.shape)
        print(self._sets)
        images_in_set = numpy.where(self._sets == _setIdx)[0]
        samples_in_set = len(images_in_set)
        print("samples_in_set: ", samples_in_set)
        ...
        print("images_tmp.shape: ", images_tmp.shape)
